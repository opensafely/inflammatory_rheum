...
*Open a log file
cap log close
log using "$logdir/incidence_cleaning_sens.log", replace
...
*Import dataset
import delimited "$projectdir/output/dataset_incidence_sens.csv", clear
...
save "$projectdir/output/data/incidence_data_processed_sens.dta", replace

*Baseline tables================================================================*/

use "$projectdir/output/data/incidence_data_processed_sens.dta", clear
...
clear *
save "$projectdir/output/data/baseline_table_rounded_sens.dta", replace emptyok
...
foreach disease in $diseases {
	...
	foreach var of varlist imd ethnicity sex `disease'_age_band {
		...
		append using "$projectdir/output/data/baseline_table_rounded_sens.dta"
		save "$projectdir/output/data/baseline_table_rounded_sens.dta", replace
		restore
	}
...
use "$projectdir/output/data/baseline_table_rounded_sens.dta", clear
export delimited using "$projectdir/output/tables/baseline_table_rounded_sens.csv", datafmt replace
...
import delimited "$projectdir/output/measures/measures_incidence_sens_`first_year'.csv", clear
save "$projectdir/output/data/measures_appended_sens.dta", replace

**Loop over diseases and years
foreach year in `years' {
	if ("`year'" != "`first_year'")  {
	import delimited "$projectdir/output/measures/measures_incidence_sens_`year'.csv", clear
	append using "$projectdir/output/data/measures_appended_sens.dta"
	save "$projectdir/output/data/measures_appended_sens.dta", replace 
	}
}
...
drop if age_band==.
keep if sex == 1 | sex == 2
save "$projectdir/output/data/measures_appended_age_sex_sens.dta", replace
restore 
...
save "$projectdir/output/data/measures_appended_imd_sens.dta", replace
restore 
...
gen denominator = round(denominator_un, 5)
drop numerator_un denominator_un
save "$projectdir/output/data/measures_appended_ethn_sens.dta", replace
restore 
...
export delimited using "$projectdir/output/tables/denominator_counts_sens.csv", datafmt replace
save "$projectdir/output/data/measures_appended_sens.dta", replace 

*Incidence rates diagnoses by month, by disease; not age/sex-standardised =================================================*/

clear *
save "$projectdir/output/data/incidence_rates_rounded_sens.dta", replace emptyok
...
foreach disease in $diseases {
	...
	**Import rounded denominators
	merge 1:1 mo_year_diagn using "$projectdir/output/data/measures_appended_sens.dta", keep(match) nogen
	...
	**Output to appended dta
	append using "$projectdir/output/data/incidence_rates_rounded_sens.dta"
	save "$projectdir/output/data/incidence_rates_rounded_sens.dta", replace  
	
	restore
}

use "$projectdir/output/data/incidence_rates_rounded_sens.dta", clear
export delimited using "$projectdir/output/tables/incidence_rates_rounded_sens.csv", datafmt replace

*Age/sex-standardised incidence rates diagnoses by year, by disease=================================*/

clear *
save "$projectdir/output/data/incidence_rates_rounded_standardised_sens.dta", replace emptyok
...
foreach disease in $diseases {
	...
	**Import denominators
	merge 1:1 year age_band sex using "$projectdir/output/data/measures_appended_age_sex_sens.dta", keep(match) nogen
	...
	**Output to appended dta
	append using "$projectdir/output/data/incidence_rates_rounded_standardised_sens.dta"
	save "$projectdir/output/data/incidence_rates_rounded_standardised_sens.dta", replace  
...
clear *
save "$projectdir/output/data/incidence_rates_rounded_imd_sens.dta", replace emptyok
...
foreach disease in $diseases {
	...
	**Import denominators
	merge 1:1 year imd using "$projectdir/output/data/measures_appended_imd_sens.dta", keep(match) nogen
	...
	**Output to appended dta
	append using "$projectdir/output/data/incidence_rates_rounded_imd_sens.dta"
	save "$projectdir/output/data/incidence_rates_rounded_imd_sens.dta", replace  
	
	restore
}

clear *
save "$projectdir/output/data/incidence_rates_rounded_ethn_sens.dta", replace emptyok
...
foreach disease in $diseases {
	...
	**Import denominators
	merge 1:1 year ethnicity using "$projectdir/output/data/measures_appended_ethn_sens.dta", keep(match) nogen
	...
	**Output to appended dta
	append using "$projectdir/output/data/incidence_rates_rounded_ethn_sens.dta"
	save "$projectdir/output/data/incidence_rates_rounded_ethn_sens.dta", replace  
	
	restore
}

**Merge in IMD and ethnicity to age and sex standardised data
use "$projectdir/output/data/incidence_rates_rounded_standardised_sens.dta", clear
merge 1:1 year disease using "$projectdir/output/data/incidence_rates_rounded_ethn_sens.dta", keep(match) nogen
merge 1:1 year disease using "$projectdir/output/data/incidence_rates_rounded_imd_sens.dta", keep(match) nogen
sort disease year
export delimited using "$projectdir/output/tables/incidence_rates_rounded_subgroups_sens.csv", datafmt replace
